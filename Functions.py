...
with open("./result/"+data_file_name, "r") as fp:
    data = json.load(fp)
...
av_iter_loss = np.array([av_iter_loss[i][:1000] for i in range(len(av_iter_loss))])
std_iter_loss = np.array([std_iter_loss[i][:1000] for i in range(len(std_iter_loss))])
av_iter_comm = np.array([av_iter_comm[i][:1000] for i in range(len(av_iter_comm))])
std_iter_comm = np.array([std_iter_comm[i][:1000] for i in range(len(std_iter_comm))])
